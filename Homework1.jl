...
# url = "https://up.enterdesk.com/edpic_source/54/8b/cc/548bccaf7de12adc3e21e5be35dd5cf2.jpg"
...
# ╔═╡ d19d8988-604a-4408-96fd-052f4b5da7e2
function redify(image)
	return value_as_color.(get_reds(image))
end
...
# ╔═╡ cacabbd9-044e-4b32-a7e4-4572317ffc54
function get_blue(pixel::AbstractRGB)
	return pixel.b
end

# ╔═╡ a7c6dfd0-7ee9-4e7f-8053-332ce857ee67
function get_greens(image::AbstractMatrix)
	return get_green.(image)
end

# ╔═╡ 72f55f5f-64d9-42e2-b4bd-df1e4af9cc94
...
# ╔═╡ 4b0ae395-bf86-4be9-bdd2-0aa2ceddea6b
function mean_matrix(matrix)
	means = [mean(matrix[ : , i]) for i in 1:size(matrix)[2]]
	return mean(means)
end

# ╔═╡ 4948fedf-c326-4c18-8995-c1bb0e06c962
a = [
	1 2 3
	4 5 6
]

# ╔═╡ 4205243a-e0e1-4d7f-9f8a-493c99e5e912
mean_matrix(a)

# ╔═╡ f6898df6-ee07-11ea-2838-fde9bc739c11
function mean_color(image)
	red = mean_matrix(get_reds(image))
	green = mean_matrix(get_greens(image))
	blue = mean_matrix(get_blues(image))
...
function noisify(x::Number, s)
	return clamp(x + 2 * s * (rand() - 0.5), 0, 1)
end
...
@bind philip_noise Slider(0:0.01:2.5, show_value=true)
...
answer_about_noise_intensity = 1.4
...
# ╔═╡ 921e1652-bf3d-4582-b0e4-62897c80dc94
@bind power Slider(0.1:0.1:3, show_value=true)

# ╔═╡ ee5f21fb-1076-42b6-8926-8bbb6ed0ad67
function custom_filter(pixel::AbstractRGB)
	return RGB(pixel.r^power, pixel.g^power, pixel.b^power)
end
...
manifest_format = "2.0"
project_hash = "1f320c048716e3c77efc0888ba521f1aae4a567b"
